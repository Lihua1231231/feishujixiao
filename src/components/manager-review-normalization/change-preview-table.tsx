...
import type { ManagerReviewNormalizationMovementRow } from "./types";
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>评分人校准</TableHead>
              <TableHead>部门校准</TableHead>
            ...
            {movementRows.map((row) => {
              ...
              const detailSummary =
                row.movementLabel === "上调"
                  ? "校准结果高于原始分桶，后续模拟排名会上升。"
                  : row.movementLabel === "下调"
                    ? "校准结果低于原始分桶，后续模拟排名会下降。"
                    : row.movementLabel === "不变"
                      ? "校准结果与原始分桶一致，排名口径没有变化。"
              ...
              return (
                <Fragment key={row.sourceRecordId}>
                  <TableRow>
                    ...
                    <TableCell>
                      {row.reviewerNormalizedBucket == null ? "—" : `${row.reviewerNormalizedBucket} 星`}
                    </TableCell>
                    <TableCell>
                      {row.departmentNormalizedBucket == null ? "—" : `${row.departmentNormalizedBucket} 星`}
                    </TableCell>
                  ...
                  {expanded ? (
                    <TableRow className="bg-muted/20">
                      <TableCell colSpan={9} className="space-y-3 py-4">
                        <p className="text-sm text-foreground">{detailSummary}</p>
                        <div className="grid gap-3 text-sm text-muted-foreground md:grid-cols-3">
                          <div>
                            <p className="font-medium text-foreground">原始分桶</p>
                            <p>{row.rawBucket == null ? "—" : `${row.rawBucket} 星`}</p>
                          </div>
                          <div>
                            <p className="font-medium text-foreground">校准后分桶</p>
                            <p>
                              {row.departmentNormalizedBucket == null
                                ? "—"
                                : `${row.departmentNormalizedBucket} 星`}
                            </p>
                          </div>
                          <div>
